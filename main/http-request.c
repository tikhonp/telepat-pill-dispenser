...
extern const char
    postman_root_cert_pem_start[] asm("_binary_postman_root_cert_pem_start");
extern const char
    postman_root_cert_pem_end[] asm("_binary_postman_root_cert_pem_end");
...
void log_bytes_buffer(char *buffer, int length) {
    for (int i = 0; i < length; i += 4) {
        ESP_LOGI(TAG,
                 "%d: " BYTE_TO_BINARY_PATTERN " " BYTE_TO_BINARY_PATTERN
                 " " BYTE_TO_BINARY_PATTERN " " BYTE_TO_BINARY_PATTERN,
                 i, BYTE_TO_BINARY(buffer[0]), BYTE_TO_BINARY(buffer[i + 2]),
                 BYTE_TO_BINARY(buffer[i + 2]), BYTE_TO_BINARY(buffer[i + 3]));

        uint32_t timestamp = 0;
        /*uint32_t timestamp = ((3 * 8) << (uint32_t)buffer[i]);*/
        timestamp = ((3 * 8) << (uint32_t)buffer[i]);
        /*timestamp = timestamp | ((2 * 8) << buffer[i+1]);*/
        /*timestamp = timestamp | ((1 * 8) << buffer[i+2]);*/
        /*timestamp = timestamp | buffer[i+3];*/
        ESP_LOGI(TAG, "TIMESTAMP: %"PRIu32, timestamp);
    }
}
...
esp_err_t _http_event_handler(esp_http_client_event_t *evt) {
    ...
    case HTTP_EVENT_ON_FINISH:
        ...
        if (output_buffer != NULL) {
            ...
            log_bytes_buffer(output_buffer, output_len);
...
static void http_rest_with_url(void) {
    ...
    // PUT
    esp_http_client_set_url(client, "http://" CONFIG_HTTP_ENDPOINT "/put");
    ...
    // PATCH
    esp_http_client_set_url(client, "http://" CONFIG_HTTP_ENDPOINT "/patch");
    ...
    // DELETE
    esp_http_client_set_url(client, "http://" CONFIG_HTTP_ENDPOINT "/delete");
    ...
    // HEAD
    esp_http_client_set_url(client, "http://" CONFIG_HTTP_ENDPOINT "/get");
...
static void http_auth_basic(void) {
     ...
     */
    esp_http_client_config_t config = {
        .url = "http://user:passwd@" CONFIG_HTTP_ENDPOINT
...
static void http_encoded_query(void) {
    ...
    uint32_t enc_len = uri_encode(query_val_enc, query_val, strlen(query_val));
    if (enc_len > 0) {
...
static void http_relative_redirect(void) {
    esp_http_client_config_t config = {
        .url = "http://" CONFIG_HTTP_ENDPOINT "/relative-redirect/3",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG,
                 "HTTP Relative path redirect Status = %d, content_length = "
                 "%" PRId64,
                 esp_http_client_get_status_code(client),
                 esp_http_client_get_content_length(client));
    } else {
...
static void http_absolute_redirect(void) {
    esp_http_client_config_t config = {
        .url = "http://" CONFIG_HTTP_ENDPOINT "/absolute-redirect/3",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG,
                 "HTTP Absolute path redirect Status = %d, content_length = "
                 "%" PRId64,
                 esp_http_client_get_status_code(client),
                 esp_http_client_get_content_length(client));
    } else {
...
static void http_absolute_redirect_manual(void) {
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG,
                 "HTTP Absolute path redirect (manual) Status = %d, "
...
static void http_redirect_to_https(void) {
    esp_http_client_config_t config = {
        .url = "http://" CONFIG_HTTP_ENDPOINT
               "/redirect-to?url=https://www.howsmyssl.com",
        .event_handler = _http_event_handler,
        .cert_pem = howsmyssl_com_root_cert_pem_start,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
...
static void http_download_chunk(void) {
    esp_http_client_config_t config = {
        .url = "http://" CONFIG_HTTP_ENDPOINT "/stream-bytes/8912",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
...
static void http_perform_as_stream_reader(void) {
    ...
    esp_http_client_config_t config = {
        .url = "http://" CONFIG_HTTP_ENDPOINT "/get",
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err;
...
static void https_async(void) {
    ...
    const char *post_data =
        "Using a Palantír requires a person with great strength of will and "
        "wisdom. The Palantíri were meant to "
        "be used by the Dúnedain to communicate throughout the Realms in "
        "Exile. During the War of the Ring, "
        "the Palantíri were used by many individuals. Sauron used the "
        "Ithil-stone to take advantage of the users "
        "of the other two stones, the Orthanc-stone and Anor-stone, but was "
        "also susceptible to deception himself.";
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_post_field(client, post_data, strlen(post_data));
    while (1) {
        err = esp_http_client_perform(client);
    ...
    config.url = "https://" CONFIG_HTTP_ENDPOINT "/get";
    ...
    if (err != ESP_OK) {
        ...
        if (content_length < 0) {
            ...
            int data_read = esp_http_client_read_response(
                client, output_buffer, MAX_HTTP_OUTPUT_BUFFER);
            if (data_read >= 0) {
                ESP_LOGI(TAG, "HTTP GET Status = %d, content_length = %" PRId64,
                         esp_http_client_get_status_code(client),
                         esp_http_client_get_content_length(client));
...
static void http_partial_download(void) {
    esp_http_client_config_t config = {
        .url =
            "https://dl.espressif.com/dl/esp-idf/ci/esp_http_client_demo.txt",
        .event_handler = _http_event_handler,
        .crt_bundle_attach = esp_crt_bundle_attach,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
